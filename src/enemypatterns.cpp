...
namespace MovementPattern {
    ...
    glm::vec3 HorizonPattern::move(glm::vec3 origin, float time) {
        if (origin.x < GameConfig::POSITION_LEFT_LIMIT || origin.x > GameConfig::POSITION_RIGHT_LIMIT)
...
namespace ShootingPattern {
    ...
    // ===================== SpiralPattern =====================
    SpiralPattern::SpiralPattern(float s, float step) : angle(-90.0f), angleStep(step) { speed = s; }
    ...
    // ===================== RandomBurstPattern =====================
    RandomBurstPattern::RandomBurstPattern(float s, int c) : count(c), rng(std::random_device{}()), dist(0.0f, 2 * glm::pi<float>()) { speed = s; }

    std::vector<std::function<glm::vec3(glm::vec3, float)>> RandomBurstPattern::fire() {
        std::vector<std::function<glm::vec3(glm::vec3, float)>> actions;

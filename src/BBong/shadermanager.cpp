...
GLuint ShaderManager::installShaders(
    const std::vector<ShaderInfo> &shaderInformations) {
  ...
  for (const auto &shaderInfo : shaderInformations) {
    ...
    if (shaderID == -1) {
      throw std::runtime_error("[SHADER] Failed to compile shader: " +
                               shaderInfo.first);
    }
  ...
  if (!success) {
    ...
    std::cerr << "[SHADER] Program linked successfully. (Program ID: "
              << program << ")" << std::endl;
  }
...
GLint ShaderManager::compileShader(const std::string shaderPath,
                                   GLenum shaderType) {
  GLint success;
  ...
  if (!shaderFile.is_open()) {
    throw std::runtime_error("[SHADER] Failed to open shader file: " +
                             shaderPath);
  }

  // Read shader source code
  std::string shaderCode(std::istreambuf_iterator<char>(shaderFile.rdbuf()),
  ...
  if (!success) {
    ...
    std::cerr << "[SHADER] " << shaderPath
              << " compiled successfully. (Shader ID: " << shaderID << ")"
              << std::endl;
  }
...
ShaderStateDrawingMethod ShaderManager::getCurrentDrawingState() {
  std::vector<GLenum> capabilitiesCheck = {
      ...
      GL_DITHER,       GL_POLYGON_OFFSET_FILL, GL_POLYGON_OFFSET_LINE,
  ...
  glGetBooleanv(GL_COLOR_WRITEMASK,
                reinterpret_cast<GLboolean *>(state.colorMask));
...
void ShaderManager::setCurrentDrawingState(
    const ShaderStateDrawingMethod &state) {
